...
import { FiFilter, FiX, FiChevronDown, FiChevronUp, FiArrowLeft } from "react-icons/fi";
...
const CategoryCourses = () => {
  ...
  // Fetch category info based on category name from URL
  useEffect(() => {
    const fetchCategoryInfo = async () => {
      try {
        setIsLoadingCategory(true);
        const response = await getDataHandler("category", {
        ...
        if (response.data && response.data.length > 0) {
          ...
          // Set the category filter automatically
          setFilters(prev => ({
            ...prev,
            category: [{ 
              value: category._id, 
  ...
  const { data: coursesData } = useQuery({
    ...
    enabled: !isLoadingCategory, // Only fetch courses when category is loaded
  });
        ...
        <div className="max-w-8xl mx-auto relative z-10">
          <div className="flex flex-col lg:flex-row items-center justify-between gap-12">
            ...
            >
              ...
              >
                <span className="block mb-3">{categoryInfo?.categoryName}</span>
...
`}
              ...
              <motion.div className="flex flex-col sm:flex-row gap-4">
                <a href="#Courses" variants={itemVariants}
                  ...
                  <motion.button className="text-center w-full">
                    Browse Courses
                  </motion.button>
                </a>
                <motion.button
            ...
            >
              <div className="relative w-full max-w-lg">
                ...
                <motion.img
                  ...
                  src={categoryInfo?.image || "https://images.unsplash.com/photo-1593642634524-b40b5baae6bb?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1000&q=80"}
      ...
      >
        <div className="flex justify-between items-center mb-8">
          ...
          >
            {categoryInfo?.categoryName} Courses
          </motion.h2>
          ...
          >
            {Object.values(filters).some(filterArray => 
              filterArray.length > 0 && 
              !(filterArray.length === 1 && filterArray[0]?.value === categoryInfo?._id)
            ) && (
              <button
        ...
        >
          {/* Hide category filter since we're already in a category */}
        ...
        {/* Active Filters - Hide the category filter since it's fixed */}
        {Object.entries(filters).some(([key, arrayValue]) => 
          arrayValue.length > 0 && key !== 'category'
        ) && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
        ...
        {courses.length === 0 && (
          ...
          >
            <h3 className="text-xl font-medium text-[#4D2C5E] mb-2">
              No courses found in {categoryInfo?.categoryName}
            </h3>
            <p className="text-gray-500 mb-4">Try adjusting your filters or browse all courses</p>
            <div className="flex gap-4 justify-center">
              ...
              <motion.button
                ...
                className="px-4 py-2 bg-[#4D2C5E] text-white rounded-lg shadow-md transition-colors"
              >
                Browse All Courses
              </motion.button>
            </div>
      ...
      <Helmet>
        <title>{categoryInfo?.categoryName} Courses | Upskillab</title>
        <meta name="description" content={`Explore ${categoryInfo?.categoryName} courses at Upskillab. Enhance your skills with specialized training programs.`} />
        <meta name="keywords" content={`${categoryInfo?.categoryName}, online courses, skill development, upskilling`} />
        <meta property="og:title" content={`${categoryInfo?.categoryName} Courses | Upskillab`} />
        <meta property="og:description" content={`Discover ${categoryInfo?.categoryName} courses at Upskillab designed to help you upskill and achieve your career goals.`} />
        <meta property="og:url" content={`https://upskillab.com/category/${categoryName}`} />
        <meta property="og:type" content="website" />
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content={`${categoryInfo?.categoryName} Courses | Upskillab`} />
        <meta name="twitter:description" content={`Explore ${categoryInfo?.categoryName} courses at Upskillab to boost your skills and career prospects.`} />
        <link rel="canonical" href={`https://upskillab.com/category/${categoryName}`} />
      </Helmet>
    </div>
  );
};
